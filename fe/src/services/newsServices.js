...
export async function createNew(files, title, content, contentEx01) {    
    ...
    let token = localStorage.getItem(constants.token_string);
    formData.append('cateId', constants.cateid_announcement);
...
export async function update(id, files, title, content, contentEx01) {    
    const formData = new FormData();
    let result = new resultModel();
    let token = localStorage.getItem(constants.token_string);
    ...
    .then((res) => {
        result.code = res.data.code;
        result.execResult = (res.data.code === constants.api_code_success);
        result.message = constants.mess_news_edit_success;
    })
    .catch((err) => {
        result.code = err.data.code;
        result.execResult = false;
...
export async function setShow(id) {
    let result = "";
    let token = localStorage.getItem(constants.token_string);
    const requestItem = {
        "id": id,
    ...
    await axios({
        url: constants.api_news_setShow, 
        method: "PUT", 
        headers: {   
            "Authorization" : `Bearer ${token}`
        }, 
...
export async function getById(id){
    ...
    await axios({
        url: constants.api_news_get_by_id, 
        method: "POST", 
        headers: {   
            "Authorization" : `Bearer ${token}`
        }, 
...
export async function getLatestAnnouncement(){
    let result = new announcementModel();
    await axios.get(constants.api_news_latest_show_announcement)
    .then(res => {
        result.id = res.data.result.id;
        result.subject = res.data.result.subject;
        result.content = res.data.result.content;
